...
bundle_files='controller.sh gate.sh lib.sh listen.mjs resolve-main.mjs verify-release.mjs wait-for-health.mjs controller-version avalon.service avalon-listen.service avalon-update.service avalon-update@.service avalon-update.timer'
...
if [ -e "$destination" ]; then
  ...
  install -m 644 "$here/lib.sh" "$here/listen.mjs" \
    "$here/resolve-main.mjs" "$here/verify-release.mjs" \
    "$here/wait-for-health.mjs" "$here/controller-version" \
    "$here/avalon.service" "$here/avalon-listen.service" \
    "$here/avalon-update.service" "$here/avalon-update@.service" \
...
mkdir -p "$unit_dir"
for unit in avalon.service avalon-listen.service avalon-update.service avalon-update@.service avalon-update.timer; do
