...
const STRINGS: Record<'en' | 'zh', Record<string, string>> = {
  en: {
    ...
    'vote.result': 'Vote result: {yes} approve, {no} reject — {outcome}',
    ...
    'err.methodNotAllowed': 'That request method is not supported here.',
    'err.unsupportedMediaType': 'That request must contain JSON.',
    ...
    'onuw.over.votesReceived': 'Votes received: {count}',
...
The lobby picks the standard deck for the number of players who have joined, and the host can change it. Whenever the player count changes, those choices reset to the defaults for the new count. The host can also switch on house rules: a random leader, hidden votes that publish only the tally, and rejection-count reset, which clears the count whenever a team is approved.`,
  ...
  zh: {
    ...
    'vote.result': '表决结果：{yes} 赞成，{no} 反对 —— {outcome}',
    'vote.approved': '通过',
    'vote.rejected': '否决',
    ...
    'err.methodNotAllowed': '此处不支持该请求方式。',
    'err.unsupportedMediaType': '该请求必须包含 JSON。',
    ...
    'onuw.over.votesReceived': '得票：{count}',
