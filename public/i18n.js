...
const STRINGS = {
  en: {
    ...
    'home.avatar': 'Player avatar',
    'home.avatarUpload': 'Upload (optional)',
    'home.avatarReplace': 'Replace photo',
    'home.avatarRemove': 'Remove',
    'home.avatarAuto': 'No upload? A matching JRPG avatar will appear after you join.',
    'home.avatarInitials': 'No upload? Your name initial will be used on this server.',
    ...
    'server.unreachable': 'Enter the HTTPS address of the server hosting your game.',
    'server.incompatible': 'This client uses protocol {expected}, but that server uses {actual}. Update the client or server before playing.',
    ...
    'server.httpsOnly': 'Enter a complete https:// server address.',
    ...
    'win.hammer': 'Five team proposals were rejected.',
    ...
    'err.avatarTooLarge': 'Choose an avatar image smaller than 8 MB.',
    ...
    'err.cannotTargetSelf': 'You cannot name yourself.',
    ...
    'test.reset': '− Remove added players',
    ...
    'conn.ok': 'Connected',
    'room.joining': 'Joining room {code}…',
    'room.rejoining': 'Rejoining room {code}…',
    'room.rejoiningHint': 'Your seat is kept while the game server comes back.',
    ...
    'onuw.info.minionIsPack': 'There are no werewolves among the players \u2014 so under the decisive vote you are the pack. Do not get caught.',
...
Five quests are attempted. Each round the leader proposes a team; everyone votes to approve or reject it. Five rejected teams over the course of the game and evil wins outright. If a team is approved, its members secretly play Success or Fail — good must play Success, evil may play either. One Fail card sinks the quest (two are needed on the fourth quest in games of 7 or more).
...
The lobby picks the standard deck for the number of players who have joined, and the host can change it. Whenever the player count changes, those choices reset to the defaults for the new count. The host can also switch on house rules: a random leader, hidden votes that publish only the tally, and rejection-count reset, which clears the count whenever a team is approved.`,
  ...
  zh: {
    ...
    'home.avatar': '玩家头像',
    'home.avatarUpload': '上传头像（可选）',
    'home.avatarReplace': '更换图片',
    'home.avatarRemove': '移除',
    'home.avatarAuto': '不上传也没关系，加入后会生成一张同风格的 JRPG 头像。',
    'home.avatarInitials': '不上传则使用昵称首字母；此服务器尚未配置自动生成。',
    ...
    'server.title': '游戏服务器',
    'server.unreachable': '请输入托管本局游戏的 HTTPS 服务器地址。',
    'server.incompatible': '当前客户端使用协议 {expected}，但该服务器使用协议 {actual}。请先更新客户端或服务器。',
    'server.label': '服务器地址',
    'server.placeholder': 'https://avalon.example.com',
    'server.connect': '连接',
    'server.connected': '服务器：{server}',
    'server.change': '更换服务器',
    'server.httpsOnly': '请输入完整的 https:// 服务器地址。',
    ...
    'err.avatarTooLarge': '请选择小于 8 MB 的头像图片。',
    ...
    'conn.ok': '已连接',
    'room.joining': '正在加入房间 {code}……',
    'room.rejoining': '正在重新加入房间 {code}……',
    'room.rejoiningHint': '服务器恢复期间会为你保留座位。',
    'room.forget': '离开该房间',
    ...
    'onuw.info.minionIsPack': '玩家中没有狼人 —— 在「一票定胜负」下，你就是那只狼。别被投出去。',
...
全场共进行五轮任务。每轮由队长提名队伍，全体投票赞成或反对；若累计五次组队流局，坏人直接获胜。队伍通过后，队员秘密出牌：好人只能出「成功」，坏人两者皆可。只要出现一张「失败」牌，任务即告失败（7 人以上的第四轮任务需要两张）。
